...
export function submissionId(value){const roots=[value,value?.job,value?.data,...(Array.isArray(value)?value:[]),...(Array.isArray(value?.jobs)?value.jobs:[]),...(Array.isArray(value?.data)?value.data:[])],ids=[...new Set(roots.map(v=>v?.id||v?.job_id).filter(id=>typeof id==='string'&&/^[a-zA-Z0-9-]{8,100}$/.test(id)))];if(ids.length!==1)throw Error('Unrecognised submission response; review required.');return ids[0];}
let lease;
try{
